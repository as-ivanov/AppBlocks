...
namespace AppBlocks.CodeGeneration.Roslyn.Common
{
	internal static class ExpressionGenerator
	{
		...
		private static ExpressionSyntax GenerateDoubleLiteralExpression(ITypeSymbol type, double value,
			bool canUseFieldReference)
		{
		...
		private static ExpressionSyntax GenerateSingleLiteralExpression(ITypeSymbol type, float value,
			bool canUseFieldReference)
		{
			if (!canUseFieldReference)
			{
			...
			IEnumerable<KeyValuePair<T, string>> constants)
		{
			foreach (var constant in constants)
			{
				if (constant.Key.Equals(value))
				{
					...
					return SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, memberAccess,
