	...
	public class MetricsCollectorInterfaceInheritanceListBuilder : IInheritanceListBuilder
	{
		...
		private static InterfaceData GetInheritedInterfaceData(int index, ITestContext context)
		{
			var methods = InterfaceMethodData.GetPossibleVariations(context.Options).ToList()
				...
				.ToArray();
			var metricsCollectorInterfaceAttributeData = new MetricsCollectorInterfaceAttributeData();
			return new InterfaceData("ITestInheritedInterface" + index, "TestNamespaceForITestInheritedInterface" + index, new AttributeData[] { metricsCollectorInterfaceAttributeData }, methods, Array.Empty<InterfaceData>(), false);
		}
	}
}